...
| P3: failures/portability | Integration covers permission-denied output, unreadable runs directory, invalid ledger root, SHA-256 commits, explicit finish repository override, and stale locks. A POSIX file-size quota forces an atomic report write failure (exit 1, no published partial report); actual ENOSPC could not be induced without a separate constrained volume. Native Windows is not a declared target: Bash and POSIX filesystem semantics are documented explicitly. |
